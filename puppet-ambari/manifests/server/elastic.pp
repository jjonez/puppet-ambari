class ambari::server::elastic() {

 file { 'copy_elastic_mpack':
   ensure => 'file',
   source => 'puppet:///modules/ambari/elasitc/elastic-mpack-0.0.0.1.tar.gz',
   path => '/tmp/',
   owner => 'root',
   group => 'root',
   mode  => '0744'
   notify => Exec['register_with_ambari'],
 } ->
 exec { 'register_with_ambari' 
   command => "/sbin/ambari-server install-mpack --mpack=/tmp/elastic-mpack-0.0.0.1.tar.gz --force",
   refreshonly => true,
   notify => Exec['restart_ambari'],
 } ->
 exec { 'restart_ambari' 
   command => "/sbin/ambari-server restart",
   refreshonly => true,
   notify => Exec['restart_ambari'],
 }

}
